import { Container, Typography, Box, Paper, Button, TextField, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
        ...
        {showSetup ? (
          <Paper elevation={3} sx={{ p: 4, maxWidth: 400, mx: 'auto' }}>
            ...
            <TextField
              ...
              value={numBoards}
              onChange={(e) => setNumBoards(Math.min(1000, Math.max(1, parseInt(e.target.value) || 1)))}
          ...
          <Box>
                    ...
                    {Array.from({ length: 6 }).map((_, rowIndex) => (
                      <Box key={rowIndex} sx={{ display: 'flex', gap: 1 }}>
                        {Array.from({ length: 5 }).map((_, colIndex) => {
                          ...
                          const status = guess ? checkGuess(guess, board.targetWord)[colIndex] : '';
                          ...
                          return (
                            <Box
                              ...
                              sx={{
                                ...
                                backgroundColor: status === 'correct' ? 'success.main' :
                                              status === 'in-word' ? 'warning.main' :
                                              status === 'not-in-word' ? 'grey.400' :
                                              'transparent',
            ...
            <Paper elevation={3} sx={{ p: 2, mt: 4, maxWidth: 400, mx: 'auto' }}>
              <TextField
                value={gameState.currentGuess}
                onChange={(e) => {
                  const value = e.target.value.toLowerCase().replace(/[^a-z]/g, '').slice(0, 5);
              ...
              <Button
                ...
                fullWidth
                onClick={handleGuess}
                disabled={gameState.currentGuess.length !== 5 || gameState.gameComplete}
                sx={{ mt: 2 }}
              >
                Guess
              </Button>
            ...
            {gameState.gameComplete && (
              <Typography variant="h5" align="center" color="success.main" sx={{ mt: 4 }}>
        ...
        <Dialog open={showHelp} onClose={() => setShowHelp(false)} maxWidth="md">
          ...
          <DialogContent>
            <Typography paragraph>
              Nordle is like Wordle, but with <strong>multiple boards</strong>! Guess the words on all boards simultaneously.
            </Typography>
            <Typography paragraph>
              After each guess, each board will give you feedback. The color of the tiles will show how close you are for that board:
            </Typography>
            <Box sx={{ mb: 2 }}>
              <Typography component="span" sx={{ color: 'success.main', fontWeight: 'bold' }}>Green</Typography>: Correct letter and position
            </Box>
            <Box sx={{ mb: 2 }}>
              <Typography component="span" sx={{ color: 'warning.main', fontWeight: 'bold' }}>Yellow</Typography>: Correct letter, wrong position
            </Box>
            <Box sx={{ mb: 2 }}>
              <Typography component="span" sx={{ color: 'grey.400', fontWeight: 'bold' }}>Gray</Typography>: Letter not in the word
            </Box>
            <Typography paragraph>
              <strong>Rules:</strong>
            </Typography>
            <ul>
              <li>Guess a word, and it applies to <strong>all boards</strong>.</li>
              <li>Each board will provide feedback based on the hidden word for that board.</li>
